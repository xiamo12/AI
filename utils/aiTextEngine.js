...
function softenParagraph(paragraph, options, profile) {
  ...
  if (intensity !== 'light') {
    text = text.replace(/，因此/g, '，所以').replace(/，然而/g, '，不过').replace(/此外，/g, '另外，')
    if (!/[我你咱们今天最近当时]/.test(text) && text.length > 50) text = `换个更具体的说法，${text}`
  }
  if (intensity === 'deep') {
    text = text.replace(/。/g, '。\n').replace(/\n{2,}/g, '\n').trim()
...
function addHumanAnchor(text, pass) {
  ...
  const prefix = pass >= 2 ? '具体来看，' : '换个更具体的说法，'
  return `${prefix}${normalized}`
}

function rewriteGenericSentence(sentence, pass) {
  let text = sentence
    .replace(/我是专注于(.{2,28})的([^，。！？；;\n]+)/g, '我平时主要写$1，也会记录自己的实践感受')
    .replace(/你有没有那种瞬间/g, '你可能也遇到过这样的时刻')
    .replace(/刷到一个([^，。！？；;\n]+)分享([^，。！？；;\n]+)/g, '看到$1聊起$2')
    .replace(/你刚准备试试/g, '你本来还想认真了解一下')
    .replace(/评论区有人说/g, '评论区的一句话却提醒你')
...
function applyRevisionPass(text, pass) {
  let revised = addHumanAnchor(text, pass)
...
function forceRewriteUnit(unit, index) {
  ...
  if (!hasSubstantiveContentChange(text, rewritten)) {
    if (/^我/.test(rewritten)) {
      rewritten = rewritten.replace(/^我/, '我后来') + '，这个细节能让画面更具体。'
    } else if (/^我们/.test(rewritten)) {
      rewritten = rewritten.replace(/^我们/, '我们后来') + '，也能看出具体行动。'
    } else if (/^[你]/.test(rewritten)) {
      rewritten = rewritten.replace(/^你/, '换成实际动作看，你') + '，这样读起来更像真实判断。'
    } else {
      const additions = [
        '这句话可以再补一个具体场景，读者更容易代入。',
        '这里最好说清楚对象和动作，信息会比原句更完整。',
        '如果加上一点个人判断，语气会更像真实表达。',
      ]
      rewritten = /[，,]$/.test(rewritten)
        ? `${rewritten}${additions[index % additions.length]}`
        : `${rewritten}，${additions[index % additions.length]}`
    }
  }
  return rewritten
...
function ensureChanged(original, rewritten, options = {}) {
  ...
  if (!options.forceChange) return cleanedRewrite
  ...
  if (rewritten.indexOf('，') >= 0) return cleanupRewrite(rewritten.replace('，', '。'))
  if (rewritten.indexOf('。') >= 0) return cleanupRewrite(rewritten.replace('。', '，这件事要放回具体场景里看。'))
  return cleanupRewrite(`具体来看，${rewritten}`)
}
...
function humanizeArticle(text, options = {}) {
  ...
  const rewritten = paragraphs.map((paragraph) => softenParagraph(paragraph, options, profile)).join(options.preserveStructure === false ? '\n' : '\n\n')
  const sceneText = adaptToScene(rewritten, options.scene || '通用文本')
  const pass = Number(options.pass || 1)
  const genericText = splitSentences(sceneText).map((sentence) => rewriteGenericSentence(sentence, pass)).join('。')
  const passText = applyRevisionPass(genericText || sceneText, pass)
  return {
    text: ensureChanged(normalized, passText, options),
