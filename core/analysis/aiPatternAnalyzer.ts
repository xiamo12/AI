...
export interface SentenceAnalysisWithIndex extends SentenceAnalysis {
  index: number
}
...
const DETAIL_PATTERNS: RegExp[] = [
  /\d+/,
  /今天|昨晚|早上|周末|那天|最近|刚才/,
  /手机|地铁|电脑|桌子|微信|评论区|会议室|办公室/,
  /看见|听见|翻到|坐着|站着|盯着|走到|发出去|收到/,
  /"[^"]+"|《[^》]+》/,
]

const HUMAN_VOICE_PATTERNS: RegExp[] = [
  /我|我们|你|朋友|同事|家里人/,
  /说实话|坦白讲|后来|当时|有时候|其实|我以前/,
]
...
function countHits(sentence: string, patterns: RegExp[]): number {
  return patterns.reduce((total, pattern) => total + (pattern.test(sentence) ? 1 : 0), 0)
}

function hasAbstractWords(sentence: string): boolean {
  return ABSTRACT_WORDS.some((word) => sentence.indexOf(word) >= 0)
}

function getClauseLengths(sentence: string): number[] {
  return sentence
    .split(/[，,；;、]/)
...
function getRhythmRegularity(sentence: string): number {
...
function getPrimaryProblem(scores: Record<ProblemType, number>): ProblemType {
  return (Object.keys(scores) as ProblemType[]).sort((a, b) => scores[b] - scores[a])[0]
}

// ---- 重写方向映射 ----

function getRewriteDirection(problemType: ProblemType): string {
  const directions: Record<ProblemType, string> = {
...
function analyzeSentence(sentence: string, context: Record<string, unknown> = {}): SentenceAnalysis {
  ...
  const pass = Number((context as Record<string, number>).pass || 1)
...
function analyzeArticleSentences(
  sentences: string[],
  context: Record<string, unknown> = {},
): SentenceAnalysisWithIndex[] {
