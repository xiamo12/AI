 ...
 * 采用贪心扩展：找到第一个 PAUSE 前缀句后向后扩展全部连续句，
 * 只保留第 1 句的前缀，移除后续所有句子的前缀。
 */
...
function smoothTransitions(text) {
  ...
  // 已知识别为 skill 注入后接长前缀的组合,进行模糊匹配
  // 只匹配 second = 已知填坑短语(白名单),避免误吃正文内容
