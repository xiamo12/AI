...
function buildRewriteMessages(text: string, options: RewriteOptions = {}): Message[] {
  ...
  return [
    ...
    },
    {
      ...
      content: [
        `使用场景：${scene}`,
        `优化强度：${intensityText[intensity] || intensityText['medium']}`,
...
function buildSentenceRewritePrompt(
  sentence: string,
  analysis?: { problem_type?: string; rewrite_direction?: string },
  skills?: string[],
): string {
  return [
    '请不要做同义词替换，而要改变表达路径。',
    ...
    `原句：${sentence}`,
    analysis?.problem_type ? `问题：${analysis.problem_type}` : '',
    analysis?.rewrite_direction ? `方向：${analysis.rewrite_direction}` : '',
    skills?.length ? `可用技能：${skills.join('、')}` : '',
